        ...
        if feature in features2:
            ...
            for f in features2:
                if feature in color_keywords and f in color_keywords:
                    ratio = 1 if f not in list(colors.keys()) else colors[f]
...
def search():
    ...
    for i, instance_image in enumerate(instances_list):
        pseudo_description, colors = vit_gpt2_image_to_text(vit_gpt2_model, feature_extractor, tokenizer, instance_image[0])
